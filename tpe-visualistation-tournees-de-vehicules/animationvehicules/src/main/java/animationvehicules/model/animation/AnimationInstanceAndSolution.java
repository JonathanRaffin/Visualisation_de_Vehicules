...
public class AnimationInstanceAndSolution implements IAnimation {
     ...
     */
    public AnimationInstanceAndSolution(Controller ctrl, int speed, ParserInstance pi, ParserSolution ps, ParserGeoJSON pgj,
    ...
    public void dataChecking(int time) {
        ...
        for (int i = 1; i <= nbRequest; i++) {
            if (time == this.instance.getTimeStartSource(i)) {
                ctrl.displayStartOriginTime("InstanceAndSolution", time, i);
